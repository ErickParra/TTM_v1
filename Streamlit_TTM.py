...
def load_model():
    model = TinyTimeMixerForPrediction.from_pretrained("ruta/a/tu/modelo", config="ruta/a/tu/config.json", safetensors="ruta/a/tu/model.safetensors")
    return model

#Escalar
df_cleaned = preprocessor.transform(df_cleaned)
...
st.write("Datos cargados en tiempo real:")
st.write(df_cleaned.head())

# Cargar el modelo
model = load_model()

# Hacer predicción con los datos preprocesados
predictions = model(df_cleaned)
...
# Asumiendo que model y df_cleaned están definidos y listos para usar
def plot_results(model, data, channel):
    fig, axs = plt.subplots(figsize=(10, 6))
    plot_predictions(model=model, dset=data, plot_dir=".", plot_prefix="streamlit_plot", channel=channel, ax=axs)
    return fig

# En tu código Streamlit después de cargar el modelo y los datos
if st.button('Mostrar Predicciones y Datos Reales'):
    fig = plot_results(model, df_cleaned, channel=8)  # Asegúrate de ajustar el canal según tus datos
    st.pyplot(fig)